...
# Note: Check 'outputwrap' functions in source 'qmd' 
#   notebook, for further insight on output display 
...
#| label: data-skimming 
#| echo: false
outputwrap1(list_datasets(), set_width = 45)
...
#			Note: This one will also be used
		   kwargs={"version": 0}, width=50) 
```
...
* From *'genome-functions.R'*:
	* **kN.M_prod** - \textit{KSG \underline{Prod}uct}: Its obtained by multiplying \fontnimbussnn{\textbf{
	...
	zero* when the sequence is either devoid of C/G nucleotides (e.g. \textbf{\fontnimbussnn{gc\_percentage('ATTA')
	=0}}), or lacks nucleotide diversity (e.g. \textbf{\fontnimbussnn{shannon\_entropy('GGGG')=0}}). 
...
pe_arr <- function(seq_data)
  return(array(seq_data, dim = c((ldf-6)/4, 2),
    dimnames = list(1:((ldf-6)/4), c("prom","enha"))))
